...
# let's look at relationship between cost and revenue

fig, axes = plt.subplots(figsize=(11, 7), dpi=120)
width = 0.4
x = np.arange(len(data_cost_groped))
axes[0].bar(x - width/2, data_cost_groped['sum'], width, color='b', label='Cost')
axes[0].bar(x + width/2, data_cost_groped_r['sum'], width, color='g', label='Revenue')
axes[0].set_ylabel('Sum')
axes[0].set_title('Sum Cost and Revenue by Region')
axes[0].set_xticks(x)
axes[0].set_xticklabels(data_cost_groped.index, rotation=45, ha='right')
axes[0].legend()
axes[1].bar(x - width/2, data_cost_groped['mean'], width, color='b', label='Cost')
axes[1].bar(x + width/2, data_cost_groped_r['mean'], width, color='g', label='Revenue')
axes[1].set_ylabel('Mean')
axes[1].set_title('Mean Cost and Revenue by Region')
axes[1].set_xticks(x)
axes[1].set_xticklabels(data_cost_groped.index, rotation=45, ha='right')
axes[1].legend()
plt.xlabel('Region', fontsize=16)
plt.tight_layout()
plt.show()
